            ...
            <main className="flex-1 overflow-x-hidden p-4 sm:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 print:p-0 print:m-0 print:max-w-none">
                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 print:hidden">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-black text-foreground tracking-tight">Reportes de Producción</h1>
                        <p className="text-muted-foreground">Consolidado operativo de productos y pedidos</p>
                    </div>
                            ...
                            <div className="border-b-4 border-primary pb-8 flex flex-col md:flex-row justify-between items-start gap-6">
                                <div className="space-y-2">
                                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary text-white rounded-lg text-[10px] font-black uppercase tracking-widest uppercase">
                                        <TableIcon size={12} /> Producción
                                    </div>
                                    <h2 className="text-4xl font-black uppercase tracking-tighter">Hoja de Ruta Producción</h2>
                                ...
                                <div className="bg-muted/30 p-6 rounded-3xl border border-border/50 text-right min-w-[240px]">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Período Operativo</p>
                                    <p className="text-lg font-black">{startDate} / {endDate}</p>
                                    <p className="text-sm font-bold text-primary">{viewMode === 'general' ? 'Producción General' : 'Producción por Cliente'}</p>
                                </div>
                            </div>
                                                ...
                                                <table className="w-full text-left">
                                                    <thead className="bg-muted/20 border-b border-border">
                                                        <tr>
                                                            ...
                                                            <th className="py-5 px-8 text-right text-xs font-black uppercase tracking-widest text-muted-foreground">Unidades a Producir</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody className="divide-y divide-border/50">
                                                        {Object.values(cat.products).map(prod => (
                                                            <tr key={prod.name} className="hover:bg-muted/10 transition-colors">
                                                                ...
                                                                <td className="py-5 px-8 text-right font-black text-3xl text-primary">{prod.quantity}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                    <tfoot className="bg-primary/5">
                                                        <tr>
                                                            <td className="py-6 px-8 font-black text-primary uppercase tracking-widest text-xs">Total Producción {cat.name}</td>
                                                            <td className="py-6 px-8 text-right font-black text-primary text-2xl" colSpan={2}>
                                                                {cat.totalQuantity} unidades
                                                            </td>
                                                        </tr>
                                                    </tfoot>
                                                </table>
                                                ...
                                                <table className="w-full text-left border-collapse min-w-[1000px]">
                                                    ...
                                                    <tbody className="divide-y divide-border">
                                                        {Object.entries(catData.products).map(([prodName, prodClients]) => {
                                                            const totalProdQty = Object.values(prodClients).reduce((a, b: any) => a + b, 0);
                                                            return (
                ...
                <div className="bg-card border border-border rounded-[2.5rem] p-8 shadow-xl shadow-foreground/5 space-y-6">
                    <div className="flex items-center justify-between px-2">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-black uppercase tracking-tight">Histórico de Producción (10 Días)</h2>
                                <p className="text-xs font-medium text-muted-foreground italic">Volúmenes de fabricación registrados</p>
                            </div>
                        </div>
                    </div>
                                ...
                                {loadingHistory && !historyTable.length && (
                                    <tr>
                                        <td colSpan={6} className="py-20 text-center">
                                            ...
                                            <p className="text-sm font-black text-muted-foreground uppercase tracking-widest">Sincronizando producción...</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
